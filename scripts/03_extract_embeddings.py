...
Inputs:  data/raw/chip_manifest_y{year}.parquet
         data/chips/{region_short}/{chip_id}.npz
Output:  data/embeddings/embeddings_y{year}.npz   (chip_ids, embeddings)

Filters chips with valid_pixel_fraction below VALID_THRESHOLD before embedding,
since fully-masked chips just push noise through the encoder.
...
def main() -> None:
    ...
    manifest_path = RAW_DIR / f"chip_manifest_y{target_year}.parquet"
    ...
    log.info(
        "Kept %d chips (filtered %d below valid_fraction=%.2f or missing path)",
        len(manifest), pre_n - len(manifest), VALID_THRESHOLD,
    )
    ...
    out_path = EMB_DIR / f"embeddings_y{target_year}.npz"
